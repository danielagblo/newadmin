  ...
  list: async (search?: string): Promise<User[]> => {
      ...
      try {
        ...
        try {
          ...
          if (responseStr.length > 1000) {
            console.log(
              `📊 ... (${responseStr.length - 1000} more characters)`
            );
          }
        } catch (e) {
        ...
        if (Array.isArray(response.data) && response.data.length > 0) {
          ...
          if (
            ...
            console.log(
              `📊 Sample user from paginated response:`,
              paginatedData.results[0]
            );
          }
        }
        ...
        if (Array.isArray(response.data)) {
          console.log(
            `✅ Fetched ${response.data.length} users from ${endpoint} (array format)`
          );
          ...
          if (response.data.length === 0) {
            console.log(
              `✅ ${endpoint} returned empty array - endpoint exists but no users found`
            );
            return [];
          }
          ...
          console.log(
            `✅ Validated ${validUsers.length} valid users out of ${response.data.length}`
          );

          if (validUsers.length === 0 && response.data.length > 0) {
            console.error(
              "❌ All users were filtered out! This should not happen. Sample user:",
              response.data[0]
            );
        ...
        if (paginatedData.results && Array.isArray(paginatedData.results)) {
          ...
          console.log(
            `✅ Fetched ${allUsers.length} users from ${endpoint} page 1 (paginated format)`
          );

          // If empty results, try next endpoint
          if (
            allUsers.length === 0 &&
            (!paginatedData.count || paginatedData.count === 0)
          ) {
            console.log(
              `⚠️ ${endpoint} returned empty paginated results - trying next endpoint...`
            );
            continue;
          }
          ...
          if (paginatedData.next) {
            ...
            while (hasMore) {
              try {
                ...
                >(endpoint, { params: pageParams });
                const pageData = pageResponse.data;

                if (pageData.results && Array.isArray(pageData.results)) {
                  // Validate users from this page - be very lenient (accept any object)
                  const validPageUsers = pageData.results.filter(
                    (user: any) => {
                      const isValid = user && typeof user === "object";
                      if (!isValid) {
                        console.warn(
                          "Invalid user object found on page",
                          currentPage,
                          ":",
                          user
                        );
                  ...
                  console.log(
                    `✅ Fetched ${validPageUsers.length} valid users from ${endpoint} page ${currentPage} (${pageData.results.length} total)`
                  );
                ...
                console.error(
                  `Error fetching ${endpoint} page ${currentPage}:`,
                  error
                );
                hasMore = false;
              }
          ...
          console.log(
            `✅ Total users fetched and validated from ${endpoint}: ${allUsers.length}`
          );
        ...
        console.warn(
          `⚠️ Unexpected response format from ${endpoint}:`,
          response.data
        );
        continue;
      } catch (error: any) {
        ...
        if (status === 401) {
          console.error(
            "💡 Authentication failed (401) - please log out and log in again"
          );
          throw new Error(
          ...
          console.error(
            "💡 Access denied (403) - you may not have permission to view users"
          );
          throw new Error(
            "Access denied. You may not have permission to view users."
          );
        }
    ...
    if (lastError) {
      ...
      if (status === 404) {
        ...
        console.error(
          "💡 Verify /api-v1/admin/users/ or /api-v1/users/ exists in API docs: https://api.oysloe.com/api/docs/#/"
        );
        throw new Error(
          `Endpoints not found. Please verify the users endpoint exists in your Django backend. Check API docs: https://api.oysloe.com/api/docs/#/`
        );
      }
    }
