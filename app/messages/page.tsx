...
export default function MessagesPage() {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      if (!Array.isArray(data) || data.length === 0) {
        console.warn(
          "Messages API returned empty/invalid data; leaving list empty"
        );
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Message",
      render: (m: Message) =>
        (m as any).content ? String((m as any).content).slice(0, 80) : "-",
    },
  ...
  return (
    <Layout>
      ...
      <Drawer
        ...
        title={selected ? `Message #${selected.id}` : "Message"}
      >
        {selected ? (
          <div>
            <div className="mb-4">
              <div>
                <strong>From:</strong>{" "}
                {(selected.sender as any)?.name ||
                  (selected.sender as any)?.email}
              </div>
              <div className="mt-2 whitespace-pre-wrap bg-gray-50 p-3 rounded">
